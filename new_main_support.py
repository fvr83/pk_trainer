...
def parse_data(mode_depth, positions_actions, pot_odds_and_stacks, actions_frequencies, combos_dict):
    ...
    for combo, data in combos_dict.items():
        ...
        combo_colors_info_dict[combo] = colors_info, (need_info, has_play_freq, has_ev, pure_shove)
...
def draw_proof_chart(combo_colors_info_dict, spot_actions_text_colors, combos_order, fold_combos):
    ...
    for row in range(matrix_size):
        for col in range(matrix_size):
            ...
            combo_letter_border_color = "#ECE9E9"
            if need_info:
            ...
            if pure_shove:
                ...
                for offset in offsets:
                    if "Q" in combo:
                        draw.text((combo_x + offset[0], combo_y + 1), combo, font=matrix_font, fill=combo_letter_border_color)
                    else:
                        draw.text((combo_x + offset[0], combo_y), combo, font=matrix_font, fill=combo_letter_border_color)
            if "Q" in combo:
